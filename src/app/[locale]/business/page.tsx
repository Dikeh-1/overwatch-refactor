...
export default async function BusinessPage({ params }: Props) {
  ...
  const steps = t.raw("implementation.steps") as Step[];
  const trust = t.raw("hero.trust") as string[];
  const stages = t.raw("hero.stages") as string[];
  const whyNowParagraphs = t("whyNow.description").split("\n\n");
  ...
  return (
    <>
      <section className="dark relative isolate overflow-hidden bg-[#0c0f15] pb-16 pt-28 text-white sm:pb-20 sm:pt-32 lg:pb-24 lg:pt-36">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_22%,rgba(255,255,255,0.09),transparent_30%),radial-gradient(circle_at_88%_36%,rgba(59,130,246,0.12),transparent_34%)]" />
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[0.94fr_1.06fr] lg:gap-16 lg:px-8">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-6 text-balance text-[clamp(2.35rem,8vw,4.65rem)] font-bold leading-[1.02] tracking-[-0.045em] text-white">
              {t("title")}
            </h1>
          ...
          <div className="mt-10 grid gap-5 lg:mt-14 lg:grid-cols-2">
            {sectors.map((sector, index) => {
              ...
              return (
                <article
                  key={sector.title}
                  className="group flex h-full flex-col rounded-[1.75rem] border border-border bg-card p-5 shadow-[0_18px_55px_rgba(2,6,23,0.07)] transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-1 hover:border-foreground/20 hover:shadow-[0_24px_70px_rgba(2,6,23,0.1)] sm:p-7"
                >
                  <div className="flex items-start justify-between gap-4">
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-border bg-primary-darker/45 text-foreground">
                  ...
                  <h3 className="mt-6 text-balance text-xl font-bold leading-snug text-foreground sm:text-2xl">
                    {sector.title}
                  </h3>
                  <p className="mt-3 text-sm leading-relaxed text-muted sm:text-base">
                    {sector.description}
                  </p>
                  <p className="mt-6 text-xs font-bold uppercase tracking-[0.14em] text-foreground/65">
                    {sector.helpsMonitor}
                  </p>
                  <ul className="mt-4 grid gap-x-5 gap-y-2.5 sm:grid-cols-2">
                    {sector.bullets.map((bullet) => (
                      <li key={bullet} className="flex items-start gap-2.5 text-sm leading-relaxed text-foreground/78">
                        <CheckCircle2 size={15} className="mt-0.5 shrink-0 text-foreground/60" aria-hidden="true" />
                        <span>{bullet}</span>
                      </li>
                    ))}
                  </ul>
      ...
      <section className="dark relative overflow-hidden border-y border-white/8 bg-[#11141b] py-16 text-white sm:py-20 lg:py-24">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_84%_24%,rgba(59,130,246,0.1),transparent_32%)]" />
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:items-start lg:gap-16 lg:px-8">
          <div>
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/12 bg-white/[0.06] text-white/80">
              <Clock3 size={22} aria-hidden="true" />
            </span>
            <p className="mt-6 text-xs font-bold uppercase tracking-[0.18em] text-white/55">
              {t("whyNow.eyebrow")}
            </p>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.2)] backdrop-blur-sm sm:p-8">
            <div className="space-y-5 text-base leading-relaxed text-white/68 sm:text-lg">
              {whyNowParagraphs.map((paragraph) => (
      ...
      <section className="border-b border-border bg-primary-darker/35 py-16 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <ol className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {steps.map((step, index) => (
              <li
                key={step.title}
                className="relative rounded-2xl border border-border bg-background/75 p-6 shadow-sm"
              >
                <span className="text-sm font-bold tracking-[0.16em] text-foreground/60">
                  {String(index + 1).padStart(2, "0")}
                </span>
